...
test.describe("Главная страница образования", () => {
  ...
  test.describe("Блок 'Отзывы об онлайн-школах'", () => {
    test("Проверяем, что карусель перелистывания карточек отзывов в блоке Отзывы зациклена", async () => {
      await blocksMain.checkInfiniteScrollReview();
    });

    test("Проверяем, что кнопка Посмотреть все отзывы ведет на витрину отзывов /education_centers/otzyvy", async () => {
      await blocksMain.checkViewAllRewies();
    });
  });
